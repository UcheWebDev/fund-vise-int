...
const updateProfile = () => {
  ...
  server()
    .put("/enterpreneur/roles/updateProfileInformations", profileData.value)
    .then((response) => {
...
const fetchUserProfile = async () => {
  try {
    const response = await server().get("/enterpreneur/roles/profile");
    Object.assign(profileData.value, response.data.data);
    console.log(profileData.value, "loading the profile data");
  } catch (err) {
    if (err.response && err.response.status) {
      handleStatusCodeError(err.response.status);
    } else {
